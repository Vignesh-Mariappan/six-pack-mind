/* Well being first */
import WellBeingFirstOne from "./audios/Well_being_first/1-Ready_to_lose_getting_and_doing_but_not_being.mp3";
import WellBeingFirstTwo from "./audios/Well_being_first/2-Well_being_before_closed_ones.mp3";
import WellBeingFirstThree from "./audios/Well_being_first/3-Success_due_to_well_being_not_at_the_cost_of_well_being.mp3";
import WellBeingFirstFour from "./audios/Well_being_first/4-Well_being_gives_me_top_performance.mp3";
import WellBeingFirstFive from "./audios/Well_being_first/5-Balance_bhava_tough_conversations_made_easy.mp3";

/* No Fake Needs */
import NoFakeNeedsOne from "./audios/Fake_needs/1-I_will_release_myself_from_jail.mp3";
import NoFakeNeedsTwo from "./audios/Fake_needs/2-All_expectations_are_extras.mp3";
import NoFakeNeedsThree from "./audios/Fake_needs/3-I_reject_all_marketing_and_being_particular.mp3";
import NoFakeNeedsFour from "./audios/Fake_needs/4-I_will_not_universalize_like_a_baby_elephant.mp3";
import NoFakeNeedsFive from "./audios/Fake_needs/5-I_will_playfully_break_2mm_everyday.mp3";

/* No Big Deal */
import NoBigDealOne from "./audios/No_big_deal/1-Black_hole_shrinking_no_big_deal.mp3";
import NoBigDealTwo from "./audios/No_big_deal/2-I_accept_lifes_plan_for_me.mp3";
import NoBigDealThree from "./audios/No_big_deal/3-I_will_be_a_gentle_judge_and_accept_them_for_who_they_are.mp3";
import NoBigDealFour from "./audios/No_big_deal/4-I_release_the_dead_persons_weight.mp3";
import NoBigDealFive from "./audios/No_big_deal/5-I_am_not_a_fool_I_was_just_asleep_in_my_situations.mp3";

/* Avoid Limiting Habits */
import AvoidLimitingHabitsOne from "./audios/Remove_limiting_habits_and_beliefs/1-True-story-is-not-exciting-and-i-will-not-let-the-river-dry.mp3";
import AvoidLimitingHabitsTwo from "./audios/Remove_limiting_habits_and_beliefs/2-I_will_make_true_story_choice_like_a_child.mp3";
import AvoidLimitingHabitsThree from "./audios/Remove_limiting_habits_and_beliefs/3-I_acknowledge_the_ocean_of_mercy.mp3";
import AvoidLimitingHabitsFour from "./audios/Remove_limiting_habits_and_beliefs/4-I_am_significantly_thankful.mp3";
import AvoidLimitingHabitsFive from "./audios/Remove_limiting_habits_and_beliefs/5-Life_will_work_out_and_I_will_focus_on_ocean_of_blessings.mp3";

/* Severely Blessed */
import SeverelyBlessedOne from "./audios/Severely_blessed/1-My_setbacks_are_my_blessings.mp3";
import SeverelyBlessedTwo from "./audios/Severely_blessed/2-There_is_no_wave_there_is_only_ocean.mp3";
import SeverelyBlessedThree from "./audios/Severely_blessed/3-I_will_treat_everything_like_a_bonus.mp3";
import SeverelyBlessedFour from "./audios/Severely_blessed/4-I_learn_during_bad_times_and_I_progress_during_good_times.mp3";


export const AUDIOS = {
    "Well Being First": [
        {
            id: "WB01",
            audioTitle: "Ready to lose getting and doing but not being",
            audioPath: WellBeingFirstOne
        },

        {
            id: "WB02",
            audioTitle: "Well being before closed ones",
            audioPath: WellBeingFirstTwo
        },

        {
            id: "WB03",
            audioTitle: "Success due to well being",
            audioPath: WellBeingFirstThree
        },

        {
            id: "WB04",
            audioTitle: "Well being gives me top performance",
            audioPath: WellBeingFirstFour
        },

        {
            id: "WB05",
            audioTitle: "Balance bhava tough conversations made easy",
            audioPath: WellBeingFirstFive
        },
    ],
    "No Fake Needs": [
        {
            id: "NFN01",
            audioTitle: "I'll release myself from jail",
            audioPath: NoFakeNeedsOne
        },

        {
            id: "NFN02",
            audioTitle: "All expectations are extras",
            audioPath: NoFakeNeedsTwo
        },

        {
            id: "NFN03",
            audioTitle: "I reject all marketing and being particular",
            audioPath: NoFakeNeedsThree
        },

        {
            id: "NFN04",
            audioTitle: "I will not universalize like a baby elephant",
            audioPath: NoFakeNeedsFour
        },

        {
            id: "NFN05",
            audioTitle: "I'll playfully break 2mm everyday",
            audioPath: NoFakeNeedsFive
        },
    ],
    "No Big Deal": [
        {
            id: "NBD01",
            audioTitle: "Black hole shrinking no big deal",
            audioPath: NoBigDealOne
        },

        {
            id: "NBD02",
            audioTitle: "I accept life's plan for me",
            audioPath: NoBigDealTwo
        },

        {
            id: "NBD03",
            audioTitle: "I'll be the gentle judge and accept them for who they are",
            audioPath: NoBigDealThree
        },

        {
            id: "NBD04",
            audioTitle: "I release the dead person's weight",
            audioPath: NoBigDealFour
        },

        {
            id: "NBD05",
            audioTitle: "I'm not a fool, I was asleep in my situations",
            audioPath: NoBigDealFive
        },
    ],

    "Avoid Limiting Habits": [
        {
            id: "AVH01",
            audioTitle: "True story is not exciting and I won't let the river dry",
            audioPath: AvoidLimitingHabitsOne
        },

        {
            id: "AVH02",
            audioTitle: "I'll make true story choice like a child",
            audioPath: AvoidLimitingHabitsTwo
        },

        {
            id: "AVH03",
            audioTitle: "I acknowledge the ocean of mercy",
            audioPath: AvoidLimitingHabitsThree
        },

        {
            id: "AVH04",
            audioTitle: "I'm significantly thankful",
            audioPath: AvoidLimitingHabitsFour
        },

        {
            id: "AVH05",
            audioTitle: "Life will work out and I'll focus on ocean of blessings",
            audioPath: AvoidLimitingHabitsFive
        },
    ],
    "Severely Blessed": [
        {
            id: "SB01",
            audioTitle: "My Setbacks are my blessings",
            audioPath: SeverelyBlessedOne
        },

        {
            id: "SB02",
            audioTitle: "There is no wave, there is only ocean",
            audioPath: SeverelyBlessedTwo
        },

        {
            id: "SB03",
            audioTitle: "I will treat everything like a bonus",
            audioPath: SeverelyBlessedThree
        },

        {
            id: "SB04",
            audioTitle: "I learn during bad times and I progress during good times",
            audioPath: SeverelyBlessedFour
        },

        // {
        //     id: "SB05",
        //     audioPath: "./audios/Severely_blessed/1-My_setbacks_are_my_blessings.mp3"
        // },
    ]
}